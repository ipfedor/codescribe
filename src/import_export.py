...
def count_io_variable_mappings(xml_text):
    if not xml_text:
        return 0
    n = 0
    for m in _VARIABLE_MAP_RE.finditer(xml_text):
        if m.group(1).strip():
            n += 1
    return n


def _nearest_default_io_var(text, pos, lookback=2500):
...
def _mapping_list2_block(indent, var):
    return (
        indent
        + u'<List2 Name="Mappings">\n'
        + indent
        + u'  <Single Type="{47edf8ea-3f84-452c-b998-e18f878578d3}" Method="IArchivable">\n'
        + indent
        + u'    <Single Name="Variable" Type="string">'
        + var
        + u"</Single>\n"
        + indent
        + u'    <Single Name="Id" Type="long">-1</Single>\n'
        + indent
        + u'    <Single Name="CreateVariable" Type="bool">True</Single>\n'
        + indent
        + u'    <Single Name="DefaultVariable" Type="string"></Single>\n'
        + indent
        + u'    <Single Name="IoChannelFBInstance" Type="string"></Single>\n'
        + indent
        + u"  </Single>\n"
        + indent
        + u"</List2>"
...
def write_native_preserving_io_maps(obj, path, recursive=False):
    ...
    try:
        ...
        return False
    new_count = count_io_variable_mappings(new_text)
    prev_count = count_io_variable_mappings(previous)
    text = new_text
    if new_count == 0 and prev_count > 0:
        text = previous
        safe_print(
            u"  Kept previous I/O Variable mappings in "
            + os.path.basename(path_bytes)
            + u" (new export had 0, previous had "
            + unicode(prev_count)
            + u")"
        )
    ...
    if text != new_text:
        with io.open(path_bytes, u"w", encoding=u"utf-8") as f:
            f.write(text)
    return count_io_variable_mappings(text) > 0
...
def read_native_under_parent(path, parent_obj, host_obj=None):
    """
    ...
    live_host = _object_guid(host_obj) if host_obj is not None else None
    if host_obj is not None and not live_host:
        safe_print(
            u"Warning: cannot read HostObjectGuid for "
            + os.path.basename(original_path)
            + u" — I/O Variable mappings may be dropped"
        )
    if parent_obj is not None and not live_parent:
        safe_print(
            u"Warning: cannot read ParentGuid for "
            + os.path.basename(original_path)
            + u" — device import may fail or lose mappings"
        )
    ...
    try:
        ...
        import_path = work_path
        if live_parent or live_host:
            guid_tmp = original_path + u".codescribe_device_guid.tmp"
            if _rewrite_native_device_import_guids(
                work_path, live_parent, guid_tmp, live_host_guid=live_host
            ):
                temps.append(guid_tmp)
                import_path = guid_tmp
                safe_print(
                    u"Rewrote device import Guids for "
                    + os.path.basename(original_path)
                    + (u" (host=" + live_host + u")" if live_host else u"")
                )
            elif live_host:
                safe_print(
                    u"Guid rewrite skipped (already matched) for "
                    + os.path.basename(original_path)
                )
        ...
        try:
            ...
            if import_path == original_path:
                raise
            safe_print(
                u"Warning: prepared import failed for "
                + os.path.basename(original_path)
                + u" ("
                + unicode(e)
        ...
        for tmp in temps:
            try:
                os.remove(ensure_unicode_path(tmp))
...
def _object_guid(obj):
    """
    Best-effort object Guid for native XML rewrite.

    XS Studio / ScriptEngine may expose ``guid``, ``Guid``, or only stringify
    via ``unicode(obj.guid)`` / ``str``; missing Guid → HostObjectGuid not
    rewritten → I/O Variable mappings dropped on import.
    """
    if obj is None:
        return None
    candidates = []
    for attr in (u"guid", u"Guid", u"handle"):
        try:
            candidates.append(getattr(obj, attr))
        except Exception:
            pass
    try:
        get_guid = getattr(obj, u"get_guid", None)
        if get_guid is not None and hasattr(get_guid, u"__call__"):
            candidates.append(get_guid())
    except Exception:
        pass
    for raw in candidates:
        if raw is None:
            continue
        try:
            g = _normalize_object_guid(raw)
        except Exception:
            g = None
        if g and g != u"00000000-0000-0000-0000-000000000000":
            return g
    return None
...
def _rewrite_native_parent_guids(src_path, live_parent_guid, dest_path):
    ...
    """
    return _rewrite_native_device_import_guids(
...
def _rewrite_native_device_import_guids(
    ...
    if live_host:
        new_text = re.sub(
            ur'(<Single Name="HostObjectGuid" Type="System\.Guid">)[^<]*(</Single>)',
            ur"\g<1>" + live_host + ur"\g<2>",
            new_text,
            flags=re.IGNORECASE,
        )
    if new_text == text:
